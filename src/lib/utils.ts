...
import { getStacksBlockHeight, getTotalMempoolTx } from "./stacks.ts";
...
export async function waitUntilBlock(
  ...
  do {
    ...
    // get current block
    currentBlock = await getStacksBlockHeight().catch((err: string) => exitError(err));
    ...
    const mempoolTx = await getTotalMempoolTx().catch((err: string) => exitError(err));
